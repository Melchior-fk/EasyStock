...
const ProductComponent:React.FC<ProductComponentProps> = ({ product, add, handleAddToCart}) => {
    if(!product){
        return(
            <div className='border-2 border-base-200 p-4 rounded-3xl w-full items-center'>
                Selectionner un produit pour voir les détails
            </div>
        )
    }
    ...
    <div className='border-2 border-base-200 p-4 rounded-3xl w-full items-center'>
        <div>
            <ProductImage src={product.imageUrl} alt={product.imageUrl} 
            heightClass='h-30' widthClass='w-30' />
        </div>
